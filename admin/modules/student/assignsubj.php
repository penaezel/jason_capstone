		...
		<div class="well">
		    <form
		        action="controller.php?action=assign&studentId=<?php echo $_GET['studentId'];?>&cid=<?php echo $_GET['cid'];?>&sem=<?php echo $_GET['sem'];?>&sy=<?php echo $_GET['sy'];?>&SY=<?php $_GET['SY'];?>"
		        Method="POST">
		        <table id="example" class="table table-striped" cellspacing="0">
					...
					// 		loadresult();
					  		$mydb->setQuery("SELECT  * 
											...
											WHERE s.`COURSE_ID`= '{$courseid}' AND s.`SEMESTER`= '{$sem}' AND s.`COURSE_ID`= c.`COURSE_ID` AND s.`SUBJ_CODE` 
		        ...
		        <div class="btn-group">
		            <a href="index.php?view=subject&studentId=<?php echo (isset($studentId)) ? $studentId : 'ID' ;?>&cid=<?php echo (isset($courseid)) ? $courseid : 'cid' ;?>&sy=<?php echo (isset($sy)) ? $sy : 'sy' ;?>&sem=<?php echo (isset($sem)) ? $sem : 'sem' ;?>"
		                class="btn btn-default"><span class="glyphicon glyphicon-back"></span>Back</a>
		            <button type="submit" class="btn btn-default" name="Add"><span
		                    class="glyphicon glyphicon-plus-sign"></span>Assign Selected</button>
		        </div>
		    </form>
		</div>
		<!--End of well-->
