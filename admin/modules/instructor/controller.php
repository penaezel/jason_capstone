...
function savegrade(){
	...
	if ($_POST['finalave']<=3 AND $_POST['finalave']<=100){
		$remarks = "Passed";
	}
		...
		//multi delete using checkbox as a selector
			...
			for($i=0;$i<$key;$i++){
				...
				$intructorClass->delete($id[$i]);
					...
					message("Staff subject(s) already Deleted!","info");
...
for ($i=0; $i<$subjId; $i++){
	...
	message("Staff Load(s) already Added!","info");
...
function doDelete(){
	...
	for($i=0;$i<$key;$i++){
		...
		$inst->delete($id[$i]);
			...
			message("Staff name(s) already Deleted!","info");
			redirect('index.php');
}
